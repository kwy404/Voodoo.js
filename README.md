...
Read honestly. On update Voodoo sits in the same band as hand-written vanilla and is roughly
**19x faster than Alpine**, which is the fair comparison: Alpine is also HTML-first and also
interprets expressions at runtime rather than compiling them. That update figure carried a
**49.1% coefficient of variation**, so it is a range, not a ranking — we are not claiming to beat
vanilla on the strength of a noisy sample.

On create Voodoo is **sixth of seven**, ahead of Alpine but behind every compiled and Virtual-DOM
framework. On clear it is **last**. Neither is a good result and neither is hidden here.

Solid wins update by a wide margin because a compiler generates its update path. Voodoo has no
build step, and that is the trade it makes.
...
[`examples/index.html`](examples/index.html), or serve the whole repository and browse them:
...
| [Realtime chat](examples/chat-tempo-real/) | `v-socket` and `v-room` over a real WebSocket, public rooms, private messages, presence, and automatic rejoin after a reconnect. A dependency-free test server ships with it |
...
| [DevTools](examples/devtools/) | Turning the inspector on from a single script attribute |
...
| Example | What it shows |
| --- | --- |
| [Open world 3D](examples/mundo-aberto/) | A procedurally generated city in pure WebGL2, no libraries. Day/night cycle, shadows, fog, minimap and speedometer, with the HUD and live controls written as ordinary Voodoo markup |
| [Breakout](examples/jogos/breakout/) | Five levels, falling power-ups, combo counter, lives, persisted high score and sound. Playable with keyboard and touch |
| [Tetris](examples/jogos/tetris/) | Seven-bag randomiser, ghost piece, hold, wall kicks. The board is canvas; the next-piece queue is a grid of spans built by nested `v-for` |
| [Shaders](examples/shaders/) | Four raymarching scenes in WebGL2 — Mandelbulb, infinite tunnel, metaballs and an ocean — with the whole control panel generated by `v-for` over each scene's uniforms, and `v-model` wired straight to the GPU |
